    ...
    void InitializeInputFieldFont(TMP_InputField inputField)
    {
        ...
        if (textArea != null)
        {
            ...
            if (textComponent != null)
            {
                ...
                Debug.Log($"[CurrentChatUIInitializer] Set font on inputField.textComponent, color: white, fontSize: {textComponent.fontSize}");
            }
        }
        ...
        if (placeholder != null)
        {
            ...
            if (placeholderText != null)
            {
                ...
                placeholderText.fontSize = textComponent != null ? textComponent.fontSize : 18;
        ...
        if (inputFieldContainer != null)
        {
            containerImage = inputFieldContainer.GetComponent<Image>();
            if (containerImage != null)
            {
                Debug.Log($"[CurrentChatUIInitializer] InputFieldContainer Image found, original color: {containerImage.color}");
                containerImage.color = new Color(0.15f, 0.15f, 0.15f, 0.95f);
                containerImage.raycastTarget = true;
                Debug.Log($"[CurrentChatUIInitializer] Set InputFieldContainer background color to: {containerImage.color}");
                
                // 确保 InputFieldContainer 的 RectTransform 正确
                var containerRect = inputFieldContainer.GetComponent<RectTransform>();
                if (containerRect != null)
                {
                    Debug.Log($"[CurrentChatUIInitializer] InputFieldContainer rect: pos={containerRect.anchoredPosition}, size={containerRect.sizeDelta}");
                }
            }
            else
            {
                Debug.LogError($"[CurrentChatUIInitializer] InputFieldContainer does NOT have Image component!");
            }
        }
        else
        {
            Debug.LogError($"[CurrentChatUIInitializer] InputField has NO parent!");
        }
        
        // 设置 TMP_InputField 的 targetGraphic (用于显示选中/高亮状态)
        if (containerImage != null)
        {
            var targetGraphicProperty = inputField.GetType().GetProperty("targetGraphic");
            if (targetGraphicProperty != null)
            {
                targetGraphicProperty.SetValue(inputField, containerImage);
            }
        }
