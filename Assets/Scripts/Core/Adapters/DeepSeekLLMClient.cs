...
public class DeepSeekLLMClient : MonoBehaviour, ILlmClient
{
    ...
    public void Initialize(string systemPrompt, string model = null, bool enableLogging = true)
    {
        ...
        _model = model ?? config.DeepSeekModel;
    ...
    public void SendChatMessage(string userMessage)
    {
        ...
        {
            Debug.LogWarning("[DeepSeekLLMClient] Request in progress or owner is null. Waiting...");
            return;
        }
    ...
    private IEnumerator SendRequest()
    {
        ...
        using (var request = new UnityWebRequest(_apiUrl, "POST"))
        {
            ...
            {
                Debug.LogError($"[DeepSeekLLMClient] Request failed: {request.error}");
            }
            else
            {
                try
                ...
                {
                    Debug.LogError($"[DeepSeekLLMClient] Error parsing response: {ex.Message}");
                }
            }
        }
