...
public class QwenAdapter : ILlmAdapter
{
    ...
    public string ParseResponse(string jsonResponse)
    {
        ...
        {
            Debug.LogError($"[QwenAdapter] Failed to parse response: {e.Message}");
            return string.Empty;
        }
    }
    ...
    public LlmUsage ParseUsage(string jsonResponse)
    {
        ...
        {
            Debug.LogError($"[QwenAdapter] Failed to parse usage: {e.Message}");
            return new LlmUsage
            {
                PromptTokens = 0,
                CompletionTokens = 0,
    ...
    public List<ToolCall> ParseToolCalls(string jsonResponse)
    {
        ...
        {
            Debug.LogError($"[QwenAdapter] Failed to parse tool calls: {e.Message}");
        }
