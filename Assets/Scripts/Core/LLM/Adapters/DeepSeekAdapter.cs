...
public class DeepSeekAdapter : ILlmAdapter
{
    ...
    public string ParseResponse(string jsonResponse)
    {
        ...
        {
            Debug.LogError($"[DeepSeekAdapter] Failed to parse response: {e.Message}");
            return string.Empty;
        }
    }
    ...
    public LlmUsage ParseUsage(string jsonResponse)
    {
        ...
        {
            Debug.LogError($"[DeepSeekAdapter] Failed to parse usage: {e.Message}");
            return new LlmUsage
            {
                PromptTokens = 0,
                CompletionTokens = 0,
