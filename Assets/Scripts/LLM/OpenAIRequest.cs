...
public class OpenAIRequest : MonoBehaviour
{
    ...
    private IEnumerator NetworkDiagnostics()
    {
        ...
        // 测试3: OpenAI API可达性 (注释掉以减少错误日志)
        // Debug.Log("Testing OpenAI API reachability...");
        // UnityWebRequest openaiTest = UnityWebRequest.Get("https://api.openai.com/");
        // openaiTest.timeout = 15;
        // yield return openaiTest.SendWebRequest();

        // if (openaiTest.result == UnityWebRequest.Result.Success)
        // {
        //     Debug.Log("✓ OpenAI API reachable: OK");
        // }
        // else
        // {
        //     Debug.LogError("✗ OpenAI API reachable: FAILED");
        //     Debug.LogError($"Error: {openaiTest.error}");
        //     Debug.LogError($"Response Code: {openaiTest.responseCode}");
        // }
    ...
    IEnumerator PostRequest()
    {
        ...
        else if (request.responseCode == 200)
        {
            ...
            try
            {
                ...
                // 尝试匹配双代码格式 [emotion][motion]
                var doubleMatch = Regex.Match(messageContent, @"\[(\d+)\]\[(\d+)\]");
                // 尝试匹配单代码格式 [emotion]
                var singleMatch = Regex.Match(messageContent, @"\[(\d+)\]");
                ...
                {
                    // 找到双代码格式
                    emotionCode = int.Parse(doubleMatch.Groups[1].Value);
                    motionCode = int.Parse(doubleMatch.Groups[2].Value);
                    Debug.Log($"Extracted emotion code: {emotionCode}, motion code: {motionCode}");
                }
                else if (singleMatch.Success)
                {
                    // 找到单代码格式，使用默认motion代码
                    emotionCode = int.Parse(singleMatch.Groups[1].Value);
                    motionCode = 0; // 默认motion代码
                    Debug.Log($"Extracted emotion code: {emotionCode}, using default motion code: {motionCode}");
                }
                else
                {
    ...
    private IEnumerator TryAlternativeRequest(string requestBody)
    {
        ...
        if (request.result == UnityWebRequest.Result.Success && request.responseCode == 200)
        {
            ...
            try
            {
                ...
                if (doubleMatch.Success && emotionController != null)
                {
                    int emotionCode = int.Parse(doubleMatch.Groups[1].Value);
                    int motionCode = int.Parse(doubleMatch.Groups[2].Value);
                    HandlePatientResponse(messageContent, emotionCode, motionCode);
                }
                else if (singleMatch.Success && emotionController != null)
                {
                    int emotionCode = int.Parse(singleMatch.Groups[1].Value);
                    int motionCode = 0; // 默认motion代码
                    HandlePatientResponse(messageContent, emotionCode, motionCode);
                }
            }
            catch (Exception e)
    ...
    private void HandlePatientResponse(string responseText, int emotionCode, int motionCode)
    {
        ...
        // 移除自动保存，等待report生成时一起发送
        // 保存逻辑移到ScoreManager的DisplayEvaluationToUI方法中
    }
